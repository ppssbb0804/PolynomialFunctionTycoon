...
int statusCursorIndex, settingCursorIndex, storeCursorIndex; //화면에서 현재 커서(화살표) 위치
...
#pragma region STORE
			...
			if (currentKeyState[KEY_LEFT] || currentKeyState[KEY_A]) { //오른쪽으로 커서 이동
				if (storeCursorIndex > 0) {
					storeCursorIndex--;
					reload = true;
				}
			}
			if (currentKeyState[KEY_RIGHT] || currentKeyState[KEY_D]) { //왼쪽으로 커서 이동
				if (storeCursorIndex < storeCount - 1) {
					storeCursorIndex++;
					reload = true;
				}
			}
			if (currentKeyState[KEY_UP] || currentKeyState[KEY_W]) { //아래쪽으로 커서 이동
				if (storeCursorIndex > 1) {
					storeCursorIndex -= 2;
					reload = true;
				}
			}
			if (currentKeyState[KEY_DOWN] || (currentKeyState[KEY_S] && !currentKeyState[KEY_CTRL])) { //위쪽으로 커서 이동
			...
			if (currentKeyState[KEY_ENTER] && !beforeKeyState[KEY_ENTER]) {
				if (storeCursorIndex == 0) {
					//GRAPH_STORE로 이동
				}
				else {
